...
>
> **Git feature roadmap**: see `docs/GIT_FEATURES.md` for the spec + status of all 21 git features (G1–G21). All shipped 2026-05-08; the doc still records design decisions per feature.
...
- **Main process** (`src/main/`) — Node.js side: window creation, PTY lifecycle via `node-pty`, filesystem access, persistent state (`~/.ai-yard/state.json`). IPC handlers in `ipc-handlers.ts` are a barrel that delegates to per-domain modules in `src/main/ipc/`: `app.ts`, `ai-util.ts` (one-shot `callAiOnce` headless Claude helper backing G11/G12/G14 — see `docs/GIT_FEATURES.md`), `browser-view.ts` (A5 Phases 2+3; dormant `WebContentsView` owner — `Map<viewId, {view, window, cleanups}>`; positioning driven from the renderer-side ResizeObserver in `view-adapter.ts`), `fs.ts`, `git.ts` (extended 2026-05-08 with all 21 git-feature IPC channels — commit/push/pull/fetch/stash/tag/reflog/log/blame/compare/conflicts/bisect/pickaxe/grep/rebase/cherry-pick/submodule), `github.ts` (extended with PR review, CI status, repo stats), `provider.ts`, `pty.ts`, `session.ts`, `store.ts`. Long-running git ops (push/pull/fetch/rebase/submodule-update/cherry-pick/bisect-run) use a new `spawnGit` helper in `git-status.ts` rather than `execFile` so they don't time out at 5 s and can stream stderr. CLI tool behavior is abstracted via the provider system (`src/main/providers/`).
- **Preload** (`src/preload/preload.ts`) — Secure bridge exposing `window.aiyard` API via `contextBridge` with namespaces: `pty`, `session`, `store`, `fs`, `provider`, `menu`, `git` (60+ methods covering all G1–G21 git operations), `github` (extended with `prDetail/prFiles/prComments/submitReview/addComment/ciStatus/repoStats`), `ai` (one method, `callOnce(prompt, cwd?)`), `telemetry` (fire-and-forget; no-op unless prefs + env vars enable it — see `src/main/telemetry.ts` and `docs/PRIVACY.md`), `browserView` (A5 Phase 5 default path for WebContentsView; see `docs/MIGRATION_WEBVIEW.md` and `src/shared/browser-view-contract.ts`).
...
- `components/project-tab/` — Customizable Overview page driven by a gridstack.js drag-and-drop grid. `pane.ts` builds the toolbar (`+ Add Widget`, `Edit layout` toggle) + grid root. `grid.ts` wraps gridstack and owns tile chrome (header, drag handle, refresh/settings/remove buttons). Per-project layout persists at `ProjectRecord.overviewLayout` with widget records `{ id, type, x, y, w, h, config? }` — lazy-defaulted on first render to mirror the legacy 2-column layout, no migration code. Each widget is a `WidgetFactory` registered in `widgets/widget-registry.ts`; current types: `readiness`, `provider-tools` (refactors of the old columns), `github-prs`, `github-issues`, `team` (reuses `createMemberCard` from `components/team/member-card.ts`, listens to `'team-changed'`), `kanban` (reuses `createCardElement` from `components/board/board-card.ts`, groups by column, listens only to `'board-changed'` — live per-session metrics are intentionally omitted to avoid full-rerender storms; the full kanban tab covers that), `sessions` (active CLI sessions + recent archived split into two sections; filtered through `isCliSession`; click-to-focus calls `setActiveSession`, click-to-resume calls `resumeFromHistory`; subscribes to `session-activity`/`session-cost`/`session-unread` observers with surgical row updates for status/cost ticks; settings modal at `widgets/sessions-settings-modal.ts` with config in `widgets/sessions-types.ts` lets users tune `recentLimit`), `ci-status` (G16 — CheckRun list with ✓/✗/⟳/⊘/○ symbols, polls every 300 s, repo + ref auto-detected from git origin + active branch), `repo-stats` (G17 — stars/forks/issues chips, days-since-push, weekly-activity SVG sparkline, top contributors with avatars, refresh 1 hour). GitHub widgets use `window.aiyard.github.*` IPC backed by `src/main/github-cli.ts` which shells out to the user's local `gh` CLI (auto-detected; PATH is the augmented `getFullPath()` from `pty-manager`). Repo defaults to the project's git origin via `getGitRemoteUrl`; per-widget settings (`widgets/github-settings-modal.ts`) override repo, state, max items, and refresh interval. Read/unread tracking lives in `github-unread.ts` (Set + observer mirroring `session-unread.ts`); per-item lastSeen timestamps persist at `ProjectRecord.githubLastSeen`. The tab-bar surfaces unread by branching on `project-tab` to consult `hasUnreadInProject`. The PR row in `widgets/github-widgets.ts` exposes a `Review here` button next to the existing Review/Plan buttons that opens the in-app G13 PR review pane via `showPrReview` from `components/git-extras.ts`. Gridstack CSS is bundled by copying `node_modules/gridstack/dist/gridstack.min.css` to `dist/renderer/vendor/` (esbuild has no CSS loader); `<link>`ed from `index.html`. `styles/widgets.css` holds shared widget chrome.
- `components/git-actions-panel.ts` — Sidebar git surface mounted at `#git-actions-panel`, sibling to the existing `#git-panel`. Always renders when the project is a git repo (independent of file changes). Hosts: branch toolbar (fetch/pull/push with ahead/behind chips, history/compare/search/rebase/bisect launchers), commit area (G1) with subject/body textarea, char counter (yellow >50 / red >72), `--amend` checkbox that pre-fills last message, and `✨ Generate` (G11 — collects staged diff via `getDiff(area: 'staged')` for each staged file, builds a Conventional Commits prompt with last-10-commit-style examples, calls `window.aiyard.ai.callOnce`), conflict banner (renders only when `conflicted > 0` → opens G10 resolver), and four collapsible sections: stash (G4), tags (G7), reflog (G8 — collapsed by default), submodules (G21 — only when `.gitmodules` exists). Push uses `--force-with-lease` exclusively when force is required; non-fast-forward triggers a confirmation modal first. Initialised in `index.ts` via `initGitActionsPanel()` alongside `initGitPanel()`.
- `components/git-modals.ts` — Multi-modal overlay system on a shared `#git-modal-host` div. Hosts branch compare (G9 + G12 AI PR description with handoff to GitHub `compare?expand=1` URL), 3-pane conflict resolver (G10) with AI assist (G14, with lockfile shortcut + UNSURE token handling that preserves manual resolution), history search (G18 — pickaxe/grep/log-grep modes with results clickable into editor), drag-reorder interactive rebase (G19), and bisect wizard (G15 — manual mode with good/bad/reset buttons or auto-bisect via test command).
- `components/git-history-pane.ts` — Paginated commit list (`PAGE_SIZE=100`) on `#git-history-host`. SVG lane gutter (1.5 px line, larger circle for merge commits — `c.parents.length > 1`); ref chips for branches/tags. Click → fetches `git:show` and renders the diff in the right pane. Right-click context menu: cherry-pick (G20 — calls `git:cherryPick`), Create branch from here, Open in GitHub, Copy hash. Filter input narrows visible rows by message/author client-side without re-querying.
- `components/git-extras.ts` — Three lightweight panes sharing `#git-extras-host`: hunk staging (G3 — diff parser splits on `@@` headers, builds per-hunk patches via `gitStageHunk`/`gitUnstageHunk` which write to `os.tmpdir()` + `git apply --cached --whitespace=fix`), blame view (G6 — porcelain parser, gutter border-left hue HSL-interpolated by commit age), and in-app PR review (G13 — diff with inline comments, COMMENT/APPROVE/REQUEST_CHANGES submission, `✨ AI review` packs up to 30 file patches into a Claude prompt and pre-fills the review-body textarea). Wired into `git-panel.ts` context menu via "Stage hunks…" and "Blame" entries on tracked files.
- `styles/git-features.css` — All styling for the git-actions-panel, modal hosts, conflict 3-pane grid, rebase rows, blame gutter, hunk diff colors (`add` green / `del` red), CI status widget, repo stats widget, and toast. Pure CSS variables — no hardcoded colors. Imported by `styles.css` next to `git-panel.css`.
