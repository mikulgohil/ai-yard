...
//
// Run manually after editing build/icon.svg:
...
async function main() {
  ...
  await sharp(svg).resize(1024, 1024).png({ compressionLevel: 9 }).toFile(path.join(buildDir, 'icon.png'));
  ...
  for (const [name, size] of icnsLayout) {
    await sharp(svg).resize(size, size).png({ compressionLevel: 9 }).toFile(path.join(iconset, name));
  }
